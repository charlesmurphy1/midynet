...
from _midynet.proposer.label import (
    RestrictedUniformBlockProposer,
    RestrictedMixedBlockProposer,
)
...
def getGroundTruth(vertex, s, graph, blocks, label_graph, shift=1, B=None):
    ...
    for neighbor in graph.get_out_edges_of_idx(vertex):
        ...
        if vertex == neighbor.vertex_index:
            m *= 2
        est = (
            label_graph.get_edge_multiplicity_idx(s, t)
            if s < label_graph.get_size()
            else 0
        )
...
def main():
    ...
    for s in shift:
        ...
        for move in moves:
            bmove = BlockMove(
                0, *move, int(move[1] >= graph_prior.get_label_graph().get_size())
            )
            print(bmove)
